...
`VisionObjectStateSP` reuses reserved10 ID `0xcb9fd56c7057593a`; `visionObjectStateSP` keeps union ordinal `@136`; service frequency is fixed at 5 Hz. Objects are capped at 32. The worker connects directly to ROAD VisionIPC and only returns bounded metadata through a spawned-process queue.

The manager latch reads device type `tizi`, Tesla brand and the enable parameter only at the offroad-to-onroad edge. Worker retry is 1/2/4/8/30 seconds with a five-failures-in-ten-minutes session fuse. modeld pressure degrades objectd from 5 Hz to 2 Hz, then pauses it; modeld loss, overheat or 90% memory pauses immediately.
...
Results: 28 pure-Python unit tests passed; Ruff passed; Python and SConscript syntax passed; diff whitespace check passed. Tests cover preprocessing, decode/NMS/class mapping, tracker reset/velocity, schema ID/ordinal/service rate, retry/fuse, resource degradation/pause, session latch behavior, parameter defaults and forbidden control-chain imports.
