...
function drawTraffic(ctx, traffic, xScale, width, height) {
  ...
  const src = directional && sign.distance != null ? sign : (control && control.distance != null ? control : (sign && sign.distance != null ? sign : null));
  ...
  ctx.fillText(label + arrow + '  ' + distance.toFixed(0) + 'm', width / 2 + 86, py + 4);
...
function drawDrivingGeometry(geometry, data) {
  ...
  if (can.rear_vehicles?.left_live) { ctx.fillStyle='#60a5fa'; ctx.font='bold 19px sans-serif'; ctx.fillText('◀ 左后车',14,height-18); } if (can.rear_vehicles?.right_live) { ctx.fillStyle='#c084fc'; ctx.font='bold 19px sans-serif'; ctx.fillText('右后车 ▶',width-95,height-18); }
