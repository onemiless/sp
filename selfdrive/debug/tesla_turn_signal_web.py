...
function drawTraffic(ctx, traffic, xScale, width, height) {
  if (!traffic?.available) return; const distance = traffic.stop_line_distance_m ?? traffic.control_distance_m; if (distance == null || distance < 0 || distance > 100) return;
  const color = ({red:'#ef4444',green:'#22c55e',yellow:'#facc15',white:'#f8fafc'})[traffic.light_state] || ({red:'#ef4444',green:'#22c55e',yellow:'#facc15'})[traffic.road_sign_color] || '#94a3b8'; const py = height - 32 - distance * xScale;
  ctx.strokeStyle = color; ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(width / 2 - 78, py); ctx.lineTo(width / 2 + 78, py); ctx.stroke(); ctx.fillStyle = color; ctx.font = 'bold 13px sans-serif'; ctx.fillText(ct(traffic.light_state !== 'none' ? traffic.light_state : traffic.road_sign_type) + '  ' + distance.toFixed(0) + 'm', width / 2 + 86, py + 4);
...
function renderCanDetails(can) {
  ...
  const t = can.traffic || {}; add('交通控制', [t.control_available ? ct(t.control_type) + ' · ' + ct(t.light_state) : '未收到 APP 交通控制报文', t.stop_line_distance_m != null ? '停止线：' + t.stop_line_distance_m + 'm' : t.control_distance_m != null ? '控制点：' + t.control_distance_m + 'm' : '', t.control_available ? '状态：' + ct(t.state_machine) + ' · 功能 ' + ct(t.feature_state) + ' · 来源 ' + ct(t.control_source) : '', t.road_sign_available ? '标志：' + ct(t.road_sign_type) + ' · ' + ct(t.road_sign_color) + ' · ' + ct(t.road_sign_source) + ' · 箭头 ' + ct(t.road_sign_arrow) : '未收到道路标志报文', t.vision_light || t.vision_sign || t.vision_road_marking || t.vision_line ? '视觉：' + [t.vision_light&&'灯',t.vision_sign&&'标志',t.vision_road_marking&&'路面标记',t.vision_line&&'停止线'].filter(Boolean).join('/') : '', t.control_available ? '确认类型 ' + t.confirmation_type + ' · 继续原因 ' + t.continuation_reason + ' · 抑制原因 ' + t.warning_suppression_reason + ' · 不可用原因 ' + t.unavailable_reason : '', '数据源总线：' + (t.sources || []).join(' / ')]);
  const vehicles = can.vehicles || []; add('周边目标', vehicles.length ? [...vehicles.map(v => ct(v.category) + ' ' + ct(v.type) + ' #' + v.track_id + '：纵向 ' + v.x_m + 'm，横向 ' + v.y_m + 'm，相对速度值 ' + v.relative_speed), '注：相对速度的 T-CAN DBC 源未标物理单位'] : ['未收到有效目标', can.rear_vehicles?.left_current ? '左后方检测到车辆' : '', can.rear_vehicles?.right_trip ? '右后方检测到车辆' : '']);
...
function drawDrivingGeometry(geometry, data) {
  ...
  drawLine(ctx, geometry.path || [], xScale, yScale, danger ? '#ef4444' : '#22c55e', 5); const legacyTraffic=geometry.oem_traffic||{}; const traffic=can.traffic?.available?can.traffic:legacyTraffic.available?{available:true,light_state:['none','red','green','yellow'][legacyTraffic.light_color]||'unknown',stop_line_distance_m:legacyTraffic.stop_line_distance,control_type:'traffic_light'}:{}; drawTraffic(ctx, traffic, xScale, width, height);
  ...
  if (can.rear_vehicles?.left_current || can.rear_vehicles?.left_trip) { ctx.fillStyle='#60a5fa'; ctx.font='bold 19px sans-serif'; ctx.fillText('◀ 左后车',14,height-18); } if (can.rear_vehicles?.right_trip) { ctx.fillStyle='#c084fc'; ctx.font='bold 19px sans-serif'; ctx.fillText('右后车 ▶',width-95,height-18); }
