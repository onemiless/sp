...
from openpilot.selfdrive.objectd.postprocess import Detection, decode_yolox
...
class TestPostprocess(unittest.TestCase):
  def test_allowed_classes_nms_and_limit(self):
    ...
    output = np.zeros((4, 85), dtype=np.float32)
    output[0, :5] = [100, 100, 50, 50, 0.9]
    output[0, 7] = 0.9  # COCO car class index 2
    output[1, :5] = [102, 102, 50, 50, 0.8]
    output[1, 7] = 0.9
    output[2, :5] = [300, 300, 40, 60, 0.95]
    output[2, 5] = 0.8  # person
    output[3, :5] = [200, 200, 20, 20, 0.99]
